...
The `EpochDownloader` class is your gateway to accessing iEEG data. You can create a downloader using either a project name or a direct OSF project ID. At the time of writting, the package supports `fragility` and `kData` projects from UTMB, both datasets are for the analysis of seizure onset prediction. The downloader can be created as follows:
```{r downloader_predefined}
downloader <- EpochDownloader()

# Equivalently:
# downloader <- EpochDownloader(id = "fragility")
...
epoch_data <- downloader$Retrostudy_subpt01_1
...
```{r load_multiple}
# Load the first 3 datasets
selected_data <- c("Retrostudy_subpt01_1", "Retrostudy_subpt01_2")
